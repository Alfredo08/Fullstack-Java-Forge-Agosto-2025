...
public class ServicioAutores {
    ...
    public Autor obtenerUnoPorCorreo(String correo){
        return this.repositorioAutores.findByCorreo(correo);
    }

    public Autor agregarUno(Autor nuevoAutor){
        String contraseniaEncriptada = BCrypt.hashpw(nuevoAutor.getConfirmarContrasenia(),
                                                     BCrypt.gensalt());
        nuevoAutor.setContrasenia(contraseniaEncriptada);
